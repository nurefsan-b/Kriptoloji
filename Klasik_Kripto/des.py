...
def _des_block(block_bits, keys, decrypt=False):
    ...
    for i in key_order:
        new_right = _xor(left, _f_function(right, keys[i]))
        left = right
        right = new_right
    
    combined = right + left
    return _permute(combined, IP_INV)
...
def des_sifrele(metin, anahtar):
    ...
    key_bits = _str_to_bits(anahtar)
    keys = _generate_keys(key_bits)
    
    sifreli = ''
    for i in range(0, len(metin), 8):
        block = metin[i:i + 8]
        block_bits = _str_to_bits(block)
        encrypted_bits = _des_block(block_bits, keys, decrypt=False)
        sifreli += _bits_to_hex(encrypted_bits)
...
def des_desifre(sifreli_hex, anahtar):
    ...
    bits = _hex_to_bits(sifreli_hex)
    
    cozulmus = ''
    for i in range(0, len(bits), 64):
        block_bits = bits[i:i + 64]
        decrypted_bits = _des_block(block_bits, keys, decrypt=True)
        cozulmus += _bits_to_str(decrypted_bits)

...
def sha2_sifrele(mesaj):
    ...
    for i in range(0, len(mesaj), 64):
        ...
        for j in range(16, 64):
            s0 = _sag_dondur(w[j - 15], 7) ^ _sag_dondur(w[j - 15], 18) ^ _sag_kaydir(w[j - 15], 3)
            s1 = _sag_dondur(w[j - 2], 17) ^ _sag_dondur(w[j - 2], 19) ^ _sag_kaydir(w[j - 2], 10)
            w.append((w[j - 16] + s0 + w[j - 7] + s1) & 0xFFFFFFFF)
        
        a, b, c, d, e, f, g, h = h0, h1, h2, h3, h4, h5, h6, h7
        
        for j in range(64):
            S1 = _sag_dondur(e, 6) ^ _sag_dondur(e, 11) ^ _sag_dondur(e, 25)
            ch = (e & f) ^ ((~e) & g)
            temp1 = (h + S1 + ch + K[j] + w[j]) & 0xFFFFFFFF
            S0 = _sag_dondur(a, 2) ^ _sag_dondur(a, 13) ^ _sag_dondur(a, 22)
            maj = (a & b) ^ (a & c) ^ (b & c)
